...
#We are stuck after this point.


## Build a table with the final date of the month per company
##Then substract the companies from the prior month.
```{r}
occupation_month <- occupational %>%
  group_by(yearmon, cleaned_county) %>% 
  filter(date == max(date)) 

xyz <- occupation_month %>% 
  group_by(cleaned_company, yearmon, cleaned_county) %>% 
   arrange(desc(yearmon))

#how to subtract one tyson in madison in one month from a tyson in madison from another month but skip intervening months
...
sum(occupation_month$total_number_of_cases)

  #filter(total_number_of_cases == max(total_number_of_cases)) %>% 

  #for last date record per month, return the total
  #subtract from previous total
...
occupation_month <- zzz %>% 
  mutate(ratio_occupation = (total_number_of_cases/New_Cases))
