...
#Remove Oklahoma
#Zimmardi note: I cannot figure this out
#Need help removing these rows. 
```{r}
cleaned_data <- cleaned_data %>%
  cleaned_data[-c(41,58, 76, 102, 119, 142, 168, 200, 243),]
cleaned_data$cleaned_city <- str_trim(cleaned_data$cleaned_city) 
```

#Company Work------
#not sure if Dillar's and Dillard's are supposed to be the same. 
#not sure if simmons food and simmons poultry are the same
```{r}
#Thanks Sean, Kara and Nick:
cleaned_data <- cleaned_data %>%
...
#This yieled 11,448 total cases of infected employees in Arkansas workplaces. 
```{r}
companytotals <- cleaned_master %>% 
  select(company_name, cleaned_company, date, address, city, county, total_number_of_cases) %>% 
  group_by(cleaned_company, address, city,county) %>% 
...
butterball_huntsville <- cleaned_master %>% 
  select(cleaned_company, date, address, cleaned_city, cleaned_county, total_number_of_cases) %>% 
