...
cleaned_data <- cleaned_data %>%
  ...
  mutate(cleaned_city = case_when(
    ...
    str_detect(cleaned_city, "stu") ~ "stuttgart",
    str_detect(cleaned_city, "hun") ~ "huntsville",
    TRUE ~ cleaned_city
  ))
...
cleaned_data <- cleaned_data %>%
  ...
  mutate(cleaned_company = case_when(
    ...
    str_detect(cleaned_company, "gates") ~ "gates_corporation",
    str_detect(cleaned_company, "riceland") ~ "riceland_foods",
...
#Revision: 9,158 cases total cases of infected employees in Arkansas workplaces. 
#Previous: 11,103 

```{r}
companytotals <- cleaned_master %>% 
  select(company_name, cleaned_company, date, address, cleaned_address, cleaned_city, cleaned_county, total_number_of_cases) %>% 
...
monthcototals <- cleaned_master %>% 
  select(company_name, cleaned_company, date, yearmon, address, cleaned_address, cleaned_city, cleaned_county, total_number_of_cases) %>% 
  group_by(yearmon) %>% 
  filter(total_number_of_cases == max(total_number_of_cases)) %>% 
...
maxworksite_month <- cleaned_master %>% 
  select(company_name, cleaned_company, date, yearmon, address, cleaned_address, cleaned_city, cleaned_county, total_number_of_cases) %>% 
  group_by(cleaned_company, cleaned_address, cleaned_city, cleaned_county, yearmon) %>% 
  filter(total_number_of_cases == max(total_number_of_cases)) 
...
test <- cleaned_master %>% 
  ...
  ungroup() %>% 
  select(cleaned_company, cleaned_address, cleaned_city, cleaned_county, yearmon) %>% 
  group_by(cleaned_company, cleaned_address, cleaned_city, yearmon) %>% 
  count()
test
```
...
```{r}
m2axworksite_month <- cleaned_master %>% 
  select(company_name, cleaned_company, date, yearmon, address, cleaned_address, cleaned_city, cleaned_county, total_number_of_cases) %>% 
  group_by(cleaned_company, cleaned_address, cleaned_city, cleaned_county, yearmon) %>% 
  top_n(1, date) 
  
write.csv(m2axworksite_month, "maxworksite_month.csv")
```
...
#This may be out of date
#This may be out of date and replaced by m2axworksite_month
#county - company monthly totals  
```{r}
month_county <- cleaned_master %>% 
  select(yearmon, date, cleaned_company, cleaned_address, cleaned_city, cleaned_county, total_number_of_cases) %>% 
...
#a table with the highest value per month by an individual company
```{r}
month_county <- month_county %>% 
   select(yearmon, date, cleaned_company, cleaned_address, cleaned_city, cleaned_county, total_number_of_cases) %>% 
  arrange(cleaned_company, cleaned_address, cleaned_city, cleaned_county)

#write.csv(month_county, "month_county.csv")

```

#CHECK - MAY BE OUT OF DATE
#CHECK - MAY BE OUT OF DATE

#Check to see if we should build this table from m2axworksite_month instead
...
##Company analysis shows Tyson 281 (289 previously); George's 41; ConAgra 40; Simmons Foods 39 (Possible 40 with one more cleaning); Peco 37
...
#list of total cases by company

```{r}
totals <- cleaned_master %>% 
  select(cleaned_company, total_number_of_cases) %>% 
...
#sum(totals$total_number_of_cases)
...
#April 12 update: Need to redownload these from Google Sheets for updated reports
...
#NEW APRIL 12, 2021
```{r}
occupation2_25 <- rio::import("Leftovers.xlsx",which = "Occupational 2.25")
occupation3_1 <- rio::import("Leftovers.xlsx",which = "Occupational 3.1")
occupation3_4 <- rio::import("Leftovers.xlsx",which = "Occupational 3.4")
occupation3_15 <- rio::import("Leftovers.xlsx",which = "Occupational 3.15")
occupation3_18 <- rio::import("Leftovers.xlsx",which = "Occupational 3.18")
occupation3_22 <- rio::import("Leftovers.xlsx",which = "Occupational 3.22")
occupation4_8 <- rio::import("Leftovers.xlsx",which = "Occupational 4.8")
```
...
occupation2_25 <- clean_names(occupation2_25)
occupation3_1 <- clean_names(occupation3_1)
occupation3_4 <- clean_names(occupation3_4)
occupation3_15 <- clean_names(occupation3_15)
occupation3_18 <- clean_names(occupation3_18)
occupation3_22 <- clean_names(occupation3_22)
occupation4_8 <- clean_names(occupation4_8)
...
#Combine May 19, 2020 - April 8, 2021
