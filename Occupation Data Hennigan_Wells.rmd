...
```{r}
master_date <- occupation_master1 %>% 
  count(date)
...
#Names need to be further cleaned to exclude addresses within the name field.
occupation_master1 %>% 
...
#Hennigan note: is this old and no longer needed?
```{r}
industry_master %>% 
  select(date, city, county) %>% 
  group_by(date, city, county) %>% 
